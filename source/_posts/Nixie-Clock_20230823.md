...
![MAX1771 升压原理图](https://c.ibcl.us/Nixie-Clock_20230823/4.jpg)
...
在原理图中，博主用分层的方式，单独画出了辉光管段选和位选的原理图。

![辉光管段选原理图](https://c.ibcl.us/Nixie-Clock_20230823/5.jpg)

![辉光管位选原理图](https://c.ibcl.us/Nixie-Clock_20230823/6.jpg)

在辉光管位选原理图中，博主使用的是 20k 欧姆的限流电阻，**需要注意的是，这里的限流电阻功率需要比其他电阻稍大一些（如选用功率为 0.5W 的电阻），从而避免长期运行过烫而导致电阻寿命缩短**。

## 寄存器组合

根据前面的描述可以得出，总共有 16 个引脚需要控制（升压模块使能、辉光管段选三极管、辉光管位选三极管、氖灯符号三极管）。由于用于控制的单片机片上 GPIO 资源比较宝贵，不可能 16 个控制脚都接去单片机 GPIO 上，所以博主决定使用两个 74HC595 芯片级联组成一个 16 位的寄存器，从而实现只用 3 个片上 GPIO 对辉光钟进行控制。

原理图如下，博主将附上一张表格描述寄存器位数对应的功能。

![74HC595 原理图](https://c.ibcl.us/Nixie-Clock_20230823/7.jpg)
...
STC89C51 单片机带有一个串口，博主将其连接到了 BLE 蓝牙模块，从而实现透过无线蓝牙对辉光钟授时。也正巧博主不久前做过与 WebBluetooth 相关的项目，所以对这一块的开发也比较熟悉。至于蓝牙模块，博主选用的是 JDY-34-SPP。
...
截止发文时，博主已经为这个项目制作了 IN-12、QS-27 这两款辉光管的转接板，在不久的将来，博主预计还能支持更多辉光管，博主也欢迎用户自行制作转接板，并向仓库提交 PR。
...
当单片机监测到串口有数据时，则开始接收位命令（年月日时分秒等）和时间数据，并将数据写入 RTC 芯片的对应位，写入操作完成后，再往外发送应答字，指示本次授时成功，代码如下。
